...
OUT_OF_SCOPE_MESSAGE = (
    ...
    "If you have a question about a part, a model, a repair, or an order for one of those — I'm your guy."
)

# Used when query is in scope but needs no specific agent (greetings, general chat)
...
def scope_gate(state: State):
    # Step 1: cheap scope check — reject off-topic queries immediately
    ...
    if not scope_result.in_scope:
        return {
            'in_scope': False,
            'message_intent': [],
            'messages': [{'role': 'assistant', 'content': OUT_OF_SCOPE_MESSAGE}]
        }

    # Step 2: classify intent to determine which agent(s) to call
    intent_result = llm.with_structured_output(IntentClassifier).invoke([
        {'role': 'system', 'content': 'Determine which agent(s) to call based on the user query. Options: model_lookup, part_lookup, order_lookup, repairRAG. Return an empty list for greetings or general chat.'},
        {'role': 'user', 'content': state['messages'][-1].content}
    ])

    # Step 3: no specific agent needed — respond directly as Cooper (greetings, general chat)
    ...
    return {'in_scope': True, 'message_intent': intent_result.message_intent}
...
graph_builder.add_node('scope_gate', scope_gate)
...
graph_builder.add_edge(START, 'scope_gate')
# Empty intent means scope_gate already handled the response (chat or out-of-scope) — go to END
